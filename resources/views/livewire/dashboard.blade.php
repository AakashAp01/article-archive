    ...
    <div class="bg-white/5 border border-white/10 p-6 rounded-sm relative">
        ...
        <div class="overflow-x-auto" role="table" aria-label="Top Engagement Articles">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="text-[10px] uppercase text-[#666] font-mono tracking-widest border-b border-white/10" role="row">
                ...
                <tbody class="text-sm">
                    @forelse($topArticles as $index => $article)
                        <tr class="border-b border-white/5 hover:bg-white/5 transition-colors group" role="row">
                            <td class="py-3 pl-2 font-mono text-[#444] group-hover:text-white" role="cell">
                                {{ str_pad($index + 1, 2, '0', STR_PAD_LEFT) }}</td>
                            <td class="py-3" role="cell">
                                <div class="flex items-center gap-3">
                                    <div class="w-20 h-10 bg-black/50 border border-white/10 overflow-hidden shrink-0">
                                        @if ($article->thumbnail)
                                            <img src="{{ $article->thumbnail }}" alt="Thumbnail"
                                                class="w-full h-full object-cover opacity-50 group-hover:opacity-100">
                                        @else
                                    ...
                                    <span
                                        class="font-light text-gray-300 group-hover:text-accent transition-colors line-clamp-1 uppercase tracking-tight">{{ $article->title }}</span>
                                </div>
                            </td>
                            <td class="py-3 text-center" role="cell">
                                @if ($article->category)
                                    <span class="text-[9px] uppercase tracking-wider px-2 py-1 border font-display"
                            ...
                            <td class="py-3 text-center transition-colors" role="cell">
                                <div class="flex justify-center items-center gap-4">
                                    ...
                                    <div class="flex items-center gap-1.5 text-[#666] group-hover:text-red-500 transition-colors"
                                        ...
                                        <span class="text-[10px] font-mono">{{ $article->reports_count ?? 0 }}</span>
                                    </div>

                                </div>
                            </td>

                            <td class="py-3 text-right pr-2" role="cell">
                                <div class="flex items-center justify-end gap-2">
                                    <x-button variant="outline" class="!w-8 !h-8 !p-0 !bg-transparent" title="Edit Article">
                                        <a href="{{ route('article.edit', $article->id) }}" class="w-full h-full flex items-center justify-center">
                                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none"
                                                stroke="currentColor" stroke-width="2">
                                    ...
                                    <x-button variant="outline" class="!w-8 !h-8 !p-0 !bg-transparent" title="View Public">
                                        <a href="{{ route('article.show', $article->slug) }}" class="w-full h-full flex items-center justify-center">
                                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none"
                                                stroke="currentColor" stroke-width="2">
...
@script
    <script>
        document.addEventListener('livewire:initialized', () => {
            ...
            const reportsData = @json($chartReports);
