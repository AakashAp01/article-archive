...
<main class="max-w-7xl mx-auto mt-32 px-6 pb-20 relative">
    ...
    <div class="space-y-4" role="table" aria-label="Personnel Registry">
        ...
        <div
            ...
            <div class="col-span-2">Joined Date</div>
                    ...
                    <div class="col-span-1 md:col-span-2 flex md:justify-center" role="cell">
                        @if (!$user->trashed())
                            <x-badge wire:click="toggleType({{ $user->id }})" 
                                :variant="$user->type === 'admin' ? 'purple' : 'blue'" 
                                :text="$user->type" 
                                class="cursor-pointer hover:opacity-80 transition-opacity" />
                        @else
                            <x-badge :text="$user->type" variant="default" class="opacity-50" />
                        @endif
                    </div>
    ...
    @if ($isModalOpen)
        <x-modal id="userModal" 
            ...
            subtitle="Configure account parameters"
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="col-span-2 md:col-span-1">
                            <label class="block text-[10px] text-[#888] uppercase tracking-widest mb-2 font-mono">Full Name</label>
                        ...
                        <div class="col-span-2 md:col-span-1">
                            <label class="block text-[10px] text-[#888] uppercase tracking-widest mb-2 font-mono">Role Protocol</label>
                    ...
                    <div class="flex items-center gap-3 border border-white/10 p-3 bg-white/5">
                        ...
                        <label for="modalActive"
                            class="text-xs text-white font-mono uppercase tracking-wide">Account Access Active</label>
                    </div>

                    <div class="pt-4 border-t border-white/5">
                        <x-button type="submit" class="w-full">
                            <span wire:loading.remove wire:target="store">{{ $userId ? 'Update Profile' : 'Initialize User' }}</span>
    ...
    @if ($isDeleteModalOpen)
        <x-modal id="deleteModal" 
            ...
            subtitle="Permanently purge this user profile?"
            variant="danger"
            wire:click="closeModal"
            class="text-center">
